...
export default class extends Controller {
  ...
  connect() {
    ...
    const canvas = new fabric.Canvas(
      this.indexValue ? `canvas-${this.indexValue}` : "canvas"
    );
      ...
      fabric.Image.fromURL(
        ...
        (img) => {
          ...
          oImg.on("mousedblclick", () => {
            ...
            furnitureInfo.querySelector("a").classList.remove("d-none");
            if (element.url) {
              furnitureInfo.querySelector("a").href = element.url;
              furnitureInfo.querySelector("a").removeAttribute("disabled");
            } else {
              furnitureInfo.querySelector("a").classList.add("d-none");
            }

            furnitureInfo.querySelector("h3").textContent = `${element.title}`;
            furnitureInfo.querySelector(
              "p"
            ).textContent = `(${element.length} x ${element.width})cm`;
          });
        },
        { crossOrigin: "anonymous" }
      );
